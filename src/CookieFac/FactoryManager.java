...
import CookieFac.models.Factory;
import CookieFac.models.Invoice;
...
public class FactoryManager extends Thread {
    ...
    public static int totalDispatched=0,totalkilos=0,numberOfInvoice=0;
            ...
            if (invoice != null) {
                for (Cookie cookie : invoice.getCookies()) {
                    for (int i = 0; i < inventoryManager.getWarehouse().getCookies().size(); i++) {
                        ...
                        if (cookie.getCode().equals(cookieStock.getCode()) ) {
                            if (cookie.getQuantity() > cookieStock.getQuantity()) {
                            	...
                            	totalDispatched+=cookie.getQuantity();
                            	cookieStock.setQuantity(cookieStock.getQuantity()-cookie.getQuantity());
                                /*try {
                                    wait();
                                } catch (InterruptedException e) {
                                    e.printStackTrace();
    ...
    public float getEffectiveThroughput(){
        return ((float)(totalDispatched)/(float)(totalkilos))*100f;
    }
}
